...
def tag(repo, tag_name, treeish="HEAD"):
    """
    Tags a git repo at a given hash. This is just a wrapper around:
        $ git --git-dir=<repo>/.git tag <tag_name> <treeish> -m <message>
    """
    dotgitdir = os.path.join(repo, ".git")
    message = "Generated by `planex-pin release`"
    run(["git", "--git-dir=%s" % dotgitdir,
         "tag", tag_name, treeish, "-m", message])


def shortlog(repo, since, until="HEAD"):
    """
    Get a short (changelog-like) git log for a repo (excluding merge-commits)
    """
    dotgitdir = os.path.join(repo, ".git")
    cmd = ["git", "--git-dir=%s" % dotgitdir,
           "log", "--oneline", "--no-merges", "%s..%s" % (since, until)]
    git_log = run(cmd)['stdout'].strip()
    log_lines = git_log.split('\n')
    # each line is of form "<hash> <message>"; trim the hash
    messages = [l.split(' ', 1)[1] for l in log_lines]
    # prepend a '- ' to each message and reverse to get a changelog
    changelog = ["- %s\n" % m for m in messages]
    changelog.reverse()
    return changelog
...
def get_from_editor(initial_message=None):
    """
    Get the user to edit a temporary file with some optional initial contents
    using their $EDITOR.
    """
    editor = os.environ.get('EDITOR', 'vi')
    with tempfile.NamedTemporaryFile(prefix='planex-pin-release') as tmp:
        if initial_message:
            tmp.writelines(initial_message)
            tmp.flush()
            tmp.seek(0)
...
def release(args):
    ...
    Generates a new release for the specified package. It will:
        * Generate a changelog from the git log for the user to confirm
...
def parse_args_or_exit(argv=None):
    ...
    parser_release.add_argument('version', help='New version number')
